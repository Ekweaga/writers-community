...
function Navbar() {
  ...
  return (
  <>
  <nav className='flex md:justify-around items-center p-4 shadow justify-between fixed bg-white top-0 left-0 right-0 z-10' style={{zIndex:333}}>
    <div>
    ...
    <div>
        <ul className='md:flex items-center justify-around text-sm gap-[30px] hidden'>
            ...
            <li  className="cursor-pointer">{user? <span onClick={logout} className="bg-[#CC3996] text-white rounded p-2">Logout</span>:<span>Login</span>}</li>
        </ul>
    </div>
    <div
      ...
      px-7 py-2 font-medium bg-[#E23972]  top-0 duration-300 ${
        ...
        style={{zIndex:333}}>
          ...
          <li className="cursor-pointer" >
                ...
                <li>{user? <span onClick={logout} className="bg-white text-black rounded p-2">Logout</span>:<span>Login</span>}</li>
          </ul>
        </div>
  </nav>
  </>
